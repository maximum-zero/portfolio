...
# ---------------------------------------------------------------------------
# --fix 플래그 (phase-level feat/fix 구분)
# ---------------------------------------------------------------------------

@pytest.fixture
def fix_executor(tmp_project, phase_dir):
    """is_fix=True 로 생성된 StepExecutor 인스턴스."""
    with patch.object(ex, "ROOT", tmp_project):
        inst = ex.StepExecutor("0-mvp", is_fix=True)
    inst._root = str(tmp_project)
    inst._phases_dir = tmp_project / "phases"
    inst._phase_dir = phase_dir
    inst._phase_dir_name = "0-mvp"
...
class TestFixFlag:
    # --- _branch_name property ---

    def test_default_branch_name_is_feat(self, executor):
        assert executor._branch_name == "feat-mvp"

    def test_fix_branch_name_is_fix(self, fix_executor):
        assert fix_executor._branch_name == "fix-mvp"

    # --- _checkout_branch: --fix 시 fix- 브랜치 사용 ---

    def test_checkout_uses_fix_branch(self, fix_executor):
        calls = []
        def fake_git(*args):
            calls.append(args)
            if args[:2] == ("rev-parse", "--abbrev-ref"):
                return MagicMock(returncode=0, stdout="fix-mvp\n", stderr="")
            return MagicMock(returncode=0, stdout="", stderr="")
        fix_executor._run_git = fake_git
        fix_executor._checkout_branch()
        # 이미 fix-mvp 브랜치이므로 checkout 없이 바로 리턴
        assert calls[0] == ("rev-parse", "--abbrev-ref", "HEAD")
        assert len(calls) == 1

    def test_checkout_creates_fix_branch_when_not_exists(self, fix_executor):
        calls = []
        def fake_git(*args):
            calls.append(args)
            if args[:2] == ("rev-parse", "--abbrev-ref"):
        ...
        fix_executor._run_git = fake_git
        fix_executor._checkout_branch()
        checkout_call = next(c for c in calls if c[0] == "checkout")
        assert "fix-mvp" in checkout_call
        assert "-b" in checkout_call

    # --- _build_preamble: is_fix=True → fix: 커밋 예시 ---

    def test_preamble_uses_fix_commit_when_is_fix(self, fix_executor):
        result = fix_executor._build_preamble("", "")
        assert "fix(mvp):" in result
        assert "feat(mvp):" not in result

    def test_preamble_uses_feat_commit_by_default(self, executor):
        result = executor._build_preamble("", "")
        assert "feat(mvp):" in result
        assert "fix(mvp):" not in result

    def test_preamble_uses_feat_even_with_prev_error(self, executor):
        """retry(prev_error)가 있어도 is_fix=False면 feat: 사용."""
        result = executor._build_preamble("", "", prev_error="some error")
        assert "feat(mvp):" in result
        assert "fix(mvp):" not in result

    def test_preamble_uses_fix_with_prev_error_and_is_fix(self, fix_executor):
        """is_fix=True + prev_error 모두 있어도 fix: 사용."""
        result = fix_executor._build_preamble("", "", prev_error="some error")
        assert "fix(mvp):" in result

    # --- _commit_step: is_fix=True → fix: 커밋 메시지 ---

    def test_commit_step_uses_fix_msg(self, fix_executor):
        calls = []
        def fake_git(*args):
            calls.append(args)
            if args[:2] == ("diff", "--cached"):
            ...
            return MagicMock(returncode=0, stdout="", stderr="")
        fix_executor._run_git = fake_git
        fix_executor._commit_step(2, "ui")
        commit_calls = [c for c in calls if c[0] == "commit"]
        assert len(commit_calls) == 2
        assert "fix(mvp):" in commit_calls[0][2]
        assert "chore(mvp):" in commit_calls[1][2]

    def test_commit_step_default_uses_feat_msg(self, executor):
        calls = []
        def fake_git(*args):
            calls.append(args)
            if args[:2] == ("diff", "--cached"):
    ...
    # --- _squash_commits: is_fix=True → fix: squash 커밋 ---

    def _setup_squash_git(self, inst, merge_base="abc123", commit_count="3"):
        calls = []
        def fake_git(*args):
            calls.append(args)
    ...
    def test_squash_uses_fix_commit_msg(self, fix_executor):
        calls = self._setup_squash_git(fix_executor)
        fix_executor._squash_commits()
        commit_calls = [c for c in calls if c[0] == "commit"]
        assert len(commit_calls) == 1
        assert commit_calls[0][2] == "fix(mvp): complete phase"

    def test_squash_uses_feat_commit_msg_by_default(self, executor):
        calls = self._setup_squash_git(executor)
        executor._squash_commits()
        commit_calls = [c for c in calls if c[0] == "commit"]
        assert len(commit_calls) == 1
        assert commit_calls[0][2] == "feat(mvp): complete phase"

    def test_squash_uses_fix_branch_for_merge_base(self, fix_executor):
        calls = self._setup_squash_git(fix_executor)
        fix_executor._squash_commits()
        merge_base_call = next(c for c in calls if c[0] == "merge-base")
        assert "fix-mvp" in merge_base_call

    # --- main(): --fix 플래그 전달 ---

    def test_main_fix_flag_sets_is_fix(self, tmp_project, phase_dir):
        created = {}
        original_init = ex.StepExecutor.__init__

        def mock_init(self, phase_dir_name, *, auto_push=False,
                      only_step=None, squash=False, is_fix=False):
            created["is_fix"] = is_fix
            original_init(self, phase_dir_name, auto_push=auto_push,
                          only_step=only_step, squash=squash, is_fix=is_fix)

        with patch("sys.argv", ["execute.py", "0-mvp", "--fix"]):
            with patch.object(ex, "ROOT", tmp_project):
                with patch.object(ex.StepExecutor, "__init__", mock_init):
                    with patch.object(ex.StepExecutor, "run", return_value=None):
                        ex.main()

        assert created["is_fix"] is True

    def test_main_no_fix_flag_defaults_false(self, tmp_project, phase_dir):
        created = {}
        original_init = ex.StepExecutor.__init__
        ...
        assert created["is_fix"] is False
